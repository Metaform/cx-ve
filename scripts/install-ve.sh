...
HOST_OVERRIDES=(
  ...
  # Siglet flow token — so the CCM transfer-type mapping installed on each member's data plane
  # must be the external address, not an in-cluster one a participant outside this cluster could
  # never resolve.
