...
The test cases in this plan were derived in two steps. First, the normative statements (MUST/SHOULD/MAY) of the [DCP v1.0.1 specification](https://eclipse-dataspace-dcp.github.io/decentralized-claims-protocol/v1.0.1/) (published 2025-11-12) — primarily the Credential Issuance Protocol (§6), the Verifiable Presentation Protocol (§5), and Base Concepts (§4) — were enumerated and turned into implementation-independent test cases. Each case asserts only observable protocol behavior: HTTP status codes, message contents, and effects reachable through protocol endpoints. Implementation internals are out of scope. Second, every case was evaluated against the TCK source (`dcp-testcases` module, `main` at `74ae54a`, v1.2.1-SNAPSHOT: 120 declared tests, 117 executing, 3 `@Disabled`) to determine whether an existing TCK test exercises it. The **DCP §** and **TCK Test** columns record the result of this trace for each case.

The trace was last refreshed against [#211](https://github.com/eclipse-dataspacetck/dcp-tck/pull/211) (`1722edd`, `capabilityInvocation` and multi-key token validation) and [#212](https://github.com/eclipse-dataspacetck/dcp-tck/pull/212) (`d94a361`, additional issuance and delivery tests). Together they added 31 tests and gave 21 of the cases this plan had listed as needing an enhanced test a TCK test — several with a partial-coverage caveat recorded in Notes.
...
- **SI token validation edge cases** — now covered, but unevenly across endpoints: `kid` resolution and unresolvable `sub` DID (TOK-09, TOK-13) on the Storage, Offer, Request and Status APIs but not the Presentation Query API; `capabilityInvocation` enforcement and multi-key handling (TOK-10, TOK-11) on the Presentation Query API only. Completing the matrix is what remains
...
- **Access-token echo** — the `token` claim from the holder's request carried into the delivery token. Covered issuer-side (IS-DELIV-03) by `is_6_1_credentialDelivery_echoesAccessToken`; the holder-side expectation (CS-STOR-14) remains untested
- **Issuer metadata completeness and stability** — stable ids are covered (IS-META-03); `CredentialObject` completeness (IS-META-02) is covered for `id`, `type`, `credentialType`, `profile` and `bindingMethods`, leaving `credentialSchema`, `issuancePolicy` and `offerReason` unasserted
...
- **Presentation-side gaps** — the `vc.id` scope alias and status-based credential filtering (CS-PRES-11, CS-PRES-12). VP proof properties (CS-PRES-13) are now enforced across the whole scope-query suite
- **Delivered-credential verification** — the delivered credential passes full verifier-side validation with no manual fixes (IS-DELIV-04). Proof verification, `issuer`, `format` and `credentialType` are now asserted; holder binding (`credentialSubject.id` = holder DID) is not
...
- **Tag rule** — the TCK has no optional-test annotation: every test is `@MandatoryTest` and therefore part of the pass bar. It historically covered only normative requirements, so SEC-tagged cases had no TCK test by construction; `d94a361` broke that by adding tests for SEC-tagged CS-STOR-06/09/10/11, CS-OFF-06 and IS-REQ-06, which are consequently mandatory for conformance. Where Notes still cite "Tag rule", it now means only that no TCK test exists **yet**, not that none can.
...
The TCK repeats its token negatives per endpoint with a consistent variant suffix on these test families: `cs_06_05_01_credentialMessage_*` (Storage API), `cs_06_06_01_credentialOfferMessage_*` (Offer API), `is_6_4_x_credentialRequest_*` and `is_4_3_3_credentialRequest_*` (Request API), `is_6_8_x_credentialStatusRequest_*` and `is_4_3_3_credentialStatusRequest_*` (Status API), and `cs_04_03_03_*` (Presentation Query API, see CS-PRES-02). The TCK Test column below gives the variant suffix.

Coverage is no longer uniform across the five endpoints. TOK-09 and TOK-13 are present on the Storage, Offer, Request and Status APIs but **not** on the Presentation Query API; TOK-10 and TOK-11 exist **only** on the Presentation Query API. Extending each to the remaining endpoints is the outstanding work in this suite.
...
| TOK-02 | No `Authorization` header | 401 | MUST | 5.3.1, 6.4, 6.8 | `*_noAuthHeader`, `cs_05_03_01_noAuthHeader` | ✅ | | `cs_05_03_01_noAuthHeader` closed the last gap — the Presentation Query API was the only authenticated endpoint without this variant |
...
| TOK-09 | `kid` referencing a nonexistent verification method | 401 | MUST | 4.3.3 | `*_unknownKid` | ✅ | | Storage, Offer, Request and Status APIs (`cs_06_05_01_credentialMessage_unknownKid`, `cs_06_06_01_credentialOfferMessage_unknownKid`, `is_4_3_3_credentialRequest_unknownKid`, `is_4_3_3_credentialStatusRequest_unknownKid`); no variant on the Presentation Query API. The upstream key resolver fails to resolve a `kid` naming no verification method, and the holder's endpoints require the `kid` to belong to the `iss` DID, which makes that answer binding (#1063) |
| TOK-10 | Signing key lacks the `capabilityInvocation` relationship (e.g. listed only under `authentication`) | 401 — signature validity alone is insufficient | MUST | 4.3.3 | `cs_04_03_03_idTokenKidNoCapabilityInvocation` | ⚠️ | | Presentation Query API only; the other four endpoints have no variant. IH cannot express this: the `capabilityInvocation` relationship has no property on the upstream DidDocument model, so it can neither be published nor enforced. Blocked on the Connector |
| TOK-11 | No `kid` header and the DID document contains more than one verification method | 401 | MUST | 4.3.3 | `cs_04_03_03_idTokenNoKidMultipleVms` | ✅ | | Presentation Query API only; the other four endpoints have no variant. The upstream key resolver rejects an absent `kid` against a multi-key DID document, and the holder's endpoints reject a token without a `kid` outright (#1063) |
...
| TOK-13 | `sub` DID not resolvable | 401 | MUST | 4.3.3 | `*_unresolvableSubject` | ⚠️ | | Storage, Offer, Request and Status APIs (`cs_06_05_01_credentialMessage_unresolvableSubject`, `cs_06_06_01_credentialOfferMessage_unresolvableSubject`, `is_4_3_3_credentialRequest_unresolvableSubject`, `is_4_3_3_credentialStatusRequest_unresolvableSubject`); no variant on the Presentation Query API. IH's own tests cover the Storage API and the Credential Offer API; the remaining endpoints are unverified |
...
| CS-STOR-03 | Message missing required fields (`@context`, `type`, `issuerPid`, `holderPid`, `status`) | 400 | MUST | 6.5.1 | `cs_06_05_01_credentialMessage_invalidBody` | ✅ | | The test now removes each required property in turn and asserts 4xx for every one, so all five omissions are covered |
...
| CS-STOR-06 | `status=ISSUED` with empty `credentials` array | Accepted as a no-op (2xx, nothing stored, pending request unaffected) | SEC | — | `cs_06_05_01_credentialMessage_emptyCredentials` | ✅ | | `credentials` is OPTIONAL in §6.5.1, empty-array semantics unstated. TCK asserts 2xx acceptance only — nothing-stored and pending-request-unaffected not asserted. IH accepts the message as a no-op and leaves the pending request untouched; covered by its own tests |
| CS-STOR-07 | `holderPid` that matches no pending request | 4xx, nothing stored | MUST | 6.5, 6.5.1 | `cs_06_05_credentialMessage_unknownHolderPid` | ✅ | | TCK asserts the 4xx rejection; nothing-stored not asserted. IH's own tests cover both halves |
...
| CS-STOR-09 | Delivery by a *different* DID than the issuer the request was addressed to (token itself valid) | Rejected, nothing stored | SEC | — | `cs_06_05_01_credentialMessage_untrustedIssuer` | ✅ | | The message carries a valid, correctly signed SI token from a third-party DID the holder never addressed a request to. TCK asserts the 4xx rejection; nothing-stored not asserted. IH accepts credentials only from the issuer the request was addressed to; covered by its own tests |
| CS-STOR-10 | Delivery by a DID that is not a trusted issuer of the CS | Rejected, nothing stored | SEC | — | `cs_06_05_01_credentialMessage_untrustedIssuer` | ✅ | | Same test as CS-STOR-09: an issuer is trusted for a request exactly when the holder addressed that request to it, so the CS-STOR-09 check subsumes this. Nothing-stored not asserted; covered by IH's own tests |
| CS-STOR-11 | Credential payload whose proof/signature does not verify against a key from the issuer's DID document | Rejected, nothing stored | SEC | — | `cs_06_05_02_credentialMessage_unverifiableProof` | ✅ | | The credential claims the issuer DID but is signed with a key outside its DID document. TCK asserts the 4xx rejection; nothing-stored not asserted. IH verifies the JWS against a key resolved from the issuer's DID document for both token-based profiles. Credentials carrying an embedded Linked-Data proof are stored with a warning and without proof verification |
...
| CS-OFF-02 | Sparse offer (entries carry only `id`) | 2xx; CS resolves the remaining properties from the issuer's metadata | MUST | 6.6.1 | `cs_06_06_01_credentialOfferMessage_sparse` | ✅ | | TCK asserts 2xx acceptance only; resolution against metadata not asserted. IH completes sparse entries from the issuer's Issuer Metadata API; covered by its own tests |
| CS-OFF-03 | Missing `issuer` | 400 | MUST | 6.6.1 | `cs_06_06_01_credentialOfferMessage_invalidBody` | ✅ | | |
| CS-OFF-04 | Empty `credentials` array | 400 (§6.6.1 defines `credentials` as a non-empty array) | MUST | 6.6.1 | `cs_06_06_01_credentialOfferMessage_emptyCredentials` | ✅ | | IH rejects a null or empty `credentials` array in validation; covered by its own tests |
...
| CS-OFF-06 | Identical offer sent twice | Idempotent — no duplicated side effects (an offer does not obligate the CS to auto-request; acting on it is an implementation choice) | SEC | — | `cs_06_06_01_credentialOfferMessage_repeatedOffer` | ✅ | | The repeat is sent with a fresh `jti` so it is not a replay; the TCK asserts only that the response is 200–499, i.e. no server error. Absence of duplicated side effects is not asserted — it is unobservable from the offer endpoint. IH deduplicates: handling the identical offer twice initiates only one credential request; covered by its own tests |
...
| CS-REQ-04 | Issuer responds 201 with `Location` header (and no body) | CS associates the request with the status resource — subsequent behavior (polling, correlation) works off the `Location` value | MUST | 6.4.1 | | ✅ | | IH takes the issuer's process id from the last path segment of the `Location` header, falling back to the response body; covered by its own tests |
| CS-REQ-05 | CS re-sends after a crash/timeout | Idempotent: same `holderPid`, no second logical request | SEC | — | | ✅ | | Tag rule; a re-sent request keeps its `holderPid`, and the issuer's 409 is treated as acceptance rather than a failure; covered by IH's own tests |
...
| ID | Stimulus | Expected | Tag | DCP § | TCK Test | IH | DIV | Notes |
|---|---|---|---|---|---|---|---|---|
| IS-REQ-01 | Valid `CredentialRequestMessage` from an authorized holder | Exact 201 (not generic 2xx) with `Location` header pointing at the request-status resource; GET on `Location` with valid auth yields a valid `CredentialStatus`; response body not required for correlation | MUST | 6.4.1 | `is_6_4_1_credentialRequest`, `is_6_4_1_credentialRequest_unsupportedCredentialId` | ✅ | | `is_6_4_1_credentialRequest` asserts 2xx + non-empty `Location` + eventual delivery to the mock CS. `is_6_4_1_credentialRequest_unsupportedCredentialId` additionally GETs the `Location` value (absolute or endpoint-relative) with a valid token and parses a `CredentialStatus` from it, so dereferenceability is now exercised. Exact 201, rather than any 2xx, is still not asserted. IH's own tests assert 201 + `Location` |
| IS-REQ-02 | Post-acceptance failure (issuance prerequisites fail later) | 201 at receipt; failure reported through the status endpoint (`REJECTED`), not the initial response | MUST | 6.4.1, 6.5.1, 6.8.1 | | ✅ | | IH moves the process to `ERRORED` when issuance fails after acceptance and reports it as `REJECTED` on the status endpoint; covered by its own unit and e2e tests |
...
| IS-REQ-04 | Empty `credentials` array | 400 | MUST | 6.4.1 | `is_6_4_1_credentialRequest_emptyCredentials` | ✅ | | §6.4.1 marks `credentials` REQUIRED but, unlike §6.6.1's offer message, does not state non-empty — spec tightening candidate; the TCK enforces 4xx regardless. IH rejects an empty array with 400 and persists nothing; covered by its own tests |
| IS-REQ-05 | `credentials[].id` not present in `credentialsSupported` | 4xx, or 201 followed by status `REJECTED` — either way observable as a failure; never silently accepted or partially processed | MUST | 6.4.1 | `is_6_4_1_credentialRequest_unsupportedCredentialId` | ✅ | | The test accepts either outcome: a synchronous 4xx, or acceptance followed by polling `Location` until the status reads `REJECTED`. It fails if the request is ever reported `ISSUED`. IH's own tests cover this |
| IS-REQ-06 | Second request with the same `holderPid` from the same holder | No second issuance: 409 (or idempotent acceptance), never two deliveries | SEC | — | `is_6_4_1_credentialRequest_duplicateHolderPid` | ✅ | | The TCK allows either rejection or idempotent acceptance (200–499) and asserts the mock CS still holds exactly one delivery afterwards. IH's own tests cover this |
| IS-REQ-07 | Request with multiple credential ids | One process; all credentials delivered in a single `CredentialMessage`; `status` applies to the whole request (`REJECTED` if at least one credential cannot be issued — no partial issuance) | MUST | 6.5.1 | `is_6_4_1_credentialRequest`, `is_6_5_1_credentialDelivery_singleMessageForBatch` | ✅ | | Single-message delivery is now asserted: the batch test requests every supported credential id and requires exactly one `CredentialMessage` carrying all of them. Whole-request `REJECTED` semantics — no partial issuance when one credential fails — remain unasserted. IH's own e2e covers a two-id request end to end |
...
| IS-STAT-02 | Queried over the request lifecycle | `status` progresses `RECEIVED` → `ISSUED` (successful delivery) or `RECEIVED` → `REJECTED` (failure); only these three values ever appear | MUST | 6.8.1 | `is_6_8_1_credentialStatusRequest`, `is_6_8_1_credentialStatusRequest_statusProgression` | ✅ | | The progression test polls to a terminal status, records every transition, and asserts each observed value is one of the three and that no terminal status is followed by another. It does not require `RECEIVED` to be observed — a fast issuer may skip past it between polls. IH's state mapping is total |
| IS-STAT-03 | A *different* holder with a valid token queries the request | 4xx, no status data (§6.8: the IS MUST implement access control such that only the client that made the request MAY access) | MUST | 6.8 | `is_6_8_credentialStatusRequest_otherHolder` | ✅ | | The query carries a well-formed token correctly signed by a third-party DID that did not make the request, so the wrong-principal case is now distinguished from a merely invalid token. IH's own tests cover this |
...
| IS-META-02 | Inspect each `CredentialObject` | Contains **all** optional properties (`credentialType`, `bindingMethods`, `credentialSchema`, `profile`, `issuancePolicy`, `offerReason`) with meaningful, well-formed values | MUST | 6.7.1 | `is_6_7_1_issuerMetadata_credentialObjectProperties` | ✅ | | TCK asserts non-blank `id`, `type` = `CredentialObject`, `credentialType`, `profile`, and a non-empty `bindingMethods`; `credentialSchema`, `issuancePolicy` and `offerReason` are not asserted. In IH, `credentialSchema` is published from the credential definition's schema URL, and the issuance policy id is derived from the definition rather than generated per request, which also makes the object stable across fetches (IS-META-03) (#1063) |
| IS-META-03 | Fetch twice | `CredentialObject.id`s are stable — clients reference and cache them | MUST | — (implied by the resolution MUSTs in 6.4.1, 6.6.1) | `is_6_7_issuerMetadata_stableIds` | ✅ | | No explicit normative statement — spec tightening candidate; the TCK enforces it regardless by comparing the id sets of two consecutive fetches. IH ids equal credential-definition ids, stable by construction |
...
Delivery is now inspected directly. `d94a361` added `CredentialService.getReceivedCredentialMessages()`, which retains the `CredentialMessage` envelope and the SI token it arrived with instead of discarding them after validation, and a `CredentialDeliveryTest` suite that asserts against both.

| ID | Scenario | Expected | Tag | DCP § | TCK Test | IH | DIV | Notes |
|---|---|---|---|---|---|---|---|---|
| IS-DELIV-01 | Successful issuance | `POST` to the holder's `CredentialService` endpoint (from its DID document) at `/credentials`, with `issuerPid`, the holder's `holderPid`, `status=ISSUED`, and containers `{credentialType, payload, format}` | MUST | 5.2, 6.5.1, 6.5.2 | `is_6_4_1_credentialRequest`, `is_6_5_1_credentialDelivery_envelope` | ✅ | | The envelope test asserts `@context`, `type` = `CredentialMessage`, `status` = `ISSUED`, a non-null `issuerPid`, a `holderPid` correlating with the request that triggered the delivery, and at least one credential container |
| IS-DELIV-02 | SI token on delivery | `iss`=`sub`= issuer DID, `aud`= holder DID | MUST | 4.3, 4.3.3 | `is_6_4_1_credentialRequest`, `is_4_3_credentialDelivery_selfIssuedToken` | ✅ | | Enforced by the mock CS's token validation on receipt, and asserted explicitly by the delivery test: the retained token is re-parsed, its signature verified against a key resolved from the issuer's DID document, and `iss` = `sub` = issuer DID, `aud` = holder DID checked |
| IS-DELIV-03 | The holder's request token carried a `token` claim | The delivery token carries the same access token in its `token` claim | MUST | 6.1 | `is_6_1_credentialDelivery_echoesAccessToken` | ✅ | | The TCK sends a distinct access token on the request and asserts the delivery token echoes it. IH keeps the holder's access token in its vault under the issuance process id and adds it to the delivery token's `token` claim when the holder sent one; covered by its own tests. Reachable only from a holder that sends one, which IH's own holder does not (CS-STOR-14), so an IH-to-IH round trip leaves it unexercised — it matters for third-party holders and the TCK |
| IS-DELIV-04 | Verify the delivered credential | Proof verifies against a key in the issuer's DID document; `issuer` property set per the selected profile's VC data model; `credentialSubject.id` = holder DID; correct `format` and `credentialType` in the container; passes full verifier-side validation with no manual fixes | MUST | 5.4.3, 6.5.2, A.2 | `is_6_5_2_credentialDelivery_credentialsVerify` | ✅ | | Each delivered credential is now proof-verified against a key resolved from the issuer's DID document, with `format` checked against the known profile strings, `credentialType` present in the VC `type` array, and `issuer` (plain or `{id}`) equal to the issuer DID. **Holder binding — `credentialSubject.id` = holder DID — is not asserted**, so that half of the expectation still needs a supplemental test. IH's own generation tests verify signed output for both profiles |
...
| CS-PRES-02 | Suite TOK on the Presentation Query API, using SI tokens carrying VP access tokens | 401 per variant | MUST | 4.3.3, 5.3.1 | `cs_04_03_03_*` (9 variants: iss/sub binding, aud, sub vs DID id, nbf, expiry, jti replay, `capabilityInvocation`, no-`kid` multi-key) | ✅ | | This is the only endpoint covering TOK-10 and TOK-11 (added in `1722edd`), and the only one **not** covering TOK-09 or TOK-13 — the reverse of the other four endpoints. The missing-header case is covered separately by `cs_05_03_01_noAuthHeader` |
...
| CS-PRES-13 | Validate a returned VP externally: signature verifies against the holder DID document, referenced key carries the `authentication` relationship, VP holder matches the CS's participant DID | All generated VPs satisfy the verifier validation rules of §5.4.3 | MUST | 5.4.3 | `cs_05_04_*` (via `TestFixtures.parseAndVerifyPresentation`) | ✅ | | Not a new test method but a strengthening of the whole scope-query suite: `AbstractPresentationFlowTest.verifyCredentials` now passes the expected holder DID, so every successful query additionally asserts the VP's `iss` equals the CS DID, any `holder` property equals the CS DID, and the VP signing key carries the `authentication` relationship in its DID document. This raises the pass bar on tests that were previously green. IH published no verification relationships at all until #1063, so no generated VP satisfied §5.4.3; signing keys are now declared for authentication and withdrawn on revocation, and the VP's `iss` is the participant DID |
...
| RT-04 | Batch: one request for multiple credential types | All credentials arrive in one `CredentialMessage` and are individually usable | MUST | 6.5.1 | `is_6_5_1_credentialDelivery_singleMessageForBatch` | ✅ | | Single-message delivery of the whole batch is now asserted, and `is_6_5_2_credentialDelivery_credentialsVerify` verifies each delivered credential individually. What remains unexercised is usability end to end — presenting the batched credentials back out of a holder. IH's own e2e covers the batch round trip, with both credentials stored and individually correct |
...
**Totals across §§3–7** (88 cases, excluding the four per-table Suite TOK reference rows): **59 cite a TCK test**, of which **37 are fully covered** and **22 partially covered** (test listed with a caveat in Notes — supplemental assertions still needed); **29 are blank** (enhanced test required per §1.3) — plus **5 additional cases** in §8. The presentation section (§6) accounts for 11 of the fully covered cases via the TCK's Presentation Query API suite. The TCK's verifier suite is out of scope for this plan's SUTs (§6) and is not counted.

Twenty-one cases moved from blank to covered when the TCK trace was refreshed against [#211](https://github.com/eclipse-dataspacetck/dcp-tck/pull/211) and [#212](https://github.com/eclipse-dataspacetck/dcp-tck/pull/212): the SI-token edge cases (TOK-09–11, TOK-13, each on a subset of endpoints), Storage API semantics (CS-STOR-06, -07, -09, -10, -11), Offer API validation and idempotency (CS-OFF-04, -06), Credential Request validation (IS-REQ-04, -05, -06), status access control and lifecycle (IS-STAT-03, and the progression half of IS-STAT-02), issuer metadata (IS-META-02, -03), credential delivery (IS-DELIV-03, -04, plus the envelope and token halves of IS-DELIV-01/02), VP validity across the whole scope-query suite (CS-PRES-13), and single-message batch delivery (RT-04, and the same half of IS-REQ-07).

Two patterns account for most of the residual caveats. First, the Storage and Offer API negatives assert the 4xx rejection but not the "nothing stored" or "no duplicated side effects" half of their Expected column: proving that from the harness requires a follow-up presentation query against the SUT, which couples the issuance suites to the Resolution API and to SecureTokenService configuration. Second, the SI-token suite is covered unevenly by endpoint — TOK-09/13 everywhere except the Presentation Query API, TOK-10/11 only there.
...
- **P0 — blocks credible conformance for the Cofinity Issuer Service + BYOW Credential Services:** TOK-09/13 on the Presentation Query API and TOK-10/11 on the other four endpoints (completing the token matrix); IS-REQ-01 (exact 201, not any 2xx), IS-REQ-02/RT-03 (rejection flow), IS-REQ-07 (whole-request status — the single-message half is done), IS-META-02 (`credentialSchema`, `issuancePolicy`, `offerReason`), IS-DELIV-04 (holder binding), RT-05 (revocation publication); CS-STOR-02/RT-01 and PROF-01 (`vc20-bssl/jwt`); CS-STOR-05, CS-STOR-12, and the nothing-stored assertion on CS-STOR-09/10/11; CS-REQ-04.
- **P1 — production readiness:** CS-STOR-01 (holding assertion), CS-STOR-07 (nothing-stored assertion), CS-STOR-14 (holder-side access-token echo — the issuer side, IS-DELIV-03, is covered); CS-OFF-02 (resolution assertion); CS-REQ-01/02/03, CS-REQ-05/07; IS-REQ-08 (issuance policy); RT-02; BASE-01, CS-PRES-12, DSP-01 (after normative-standing clarification).
- **P2 — hardening:** remaining SEC/SHOULD/MAY cases (CS-STOR-06 no-op assertion, CS-STOR-08, CS-STOR-13, CS-OFF-06 side-effect assertion, CS-REQ-06/08, IS-REQ-09, IS-STAT-04 leak-indistinguishability, IS-META-04, IS-DELIV-05/06, IS-OFF-01–03, RT-04 end-to-end usability, RT-06/07), BASE-02, CS-PRES-11, PROF-02.
...
**Versioning note:** TCK test IDs above are from `main` at `74ae54a` (v1.2.1-SNAPSHOT). Pin the TCK release for conformance gating and re-verify the IDs against the pinned tag. Two IDs were renamed in `d94a361` to match their `@DisplayName` section numbers — `is_6_4_8_credentialRequest_invalidAud` → `is_6_4_9_…` and `is_6_8_8_credentialStatusRequest_invalidAud` → `is_6_8_9_…` — so a pin to an earlier tag needs the old names. The three `@Disabled` PD tests may be enabled upstream, which would change the pass bar.
